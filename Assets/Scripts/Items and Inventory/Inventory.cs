...
public class Inventory : MonoBehaviour//背包管理器     作用：管理整个背包。添加/移除物品，存储所有拥有的物品。   //Tip:看后缀 ItemData->Data  ItemObject->object
{
    ...
    public List<InventoryItem> equipment;     //额外添加，左边是list stash右边是list inventory，下面的东西 左边是equipment
    ...
    public ItemData_Equipment GetEquipment(EquipmentType _type) //这是一个「查询」类型的函数，用来在背包系统中查询当前某个部位是否已装备物品，并返回其数据。
    {
        ...
        foreach (KeyValuePair<ItemData_Equipment, InventoryItem> item in equipmentDictionary)
        {
            if (item.Key.equipmentType == _type)
                equipedItem = item.Key;    //光删字典没用，得把蓝图也删了
        }
    ...
    public void UseFlask()
    {
        ...
        bool canUseFlask = Time.time > lastTimeUsedFlask + flaskCooldown;//flaskCooldown初值为0，第一次使用后才赋值（例15）
